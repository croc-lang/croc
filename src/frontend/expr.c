...
void expr_drop(expr_t* self) {
    if (self->kind >= EX_VAR_ASSIGN && self->kind <= EX_BIN_MOD) binary_expr_drop(self->value);
    else if (self->kind >= EX_UNA_SUFFIX_INCR && self->kind <= EX_UNA_NEG) unary_expr_drop(self->value);
    else if (self->kind >= EX_VAR_ASSIGN && self->kind <= EX_BIN_MOD) string_drop(self->value);
    free(self);
}
